...
def train():
    ...
    for epoch in range(1, hp.num_epochs + 1):
        ...
        #记录时间
        end_time = time.time()
        print(f"Time spend in Epoch {epoch}:{end_time - start_time}s")
    writer.close()
...
def evaluate(model, epoch, writer, score_list):
    ...
    # 使用ntlk_blue1计算
    multi_bleu = 0
    bleu_1_gram = corpus_bleu(list_of_refs, hypotheses, weights=(1, 0, 0, 0))
    bleu_2_gram = 0
    bleu_3_gram = 0
    bleu_4_gram = corpus_bleu(list_of_refs, hypotheses, weights=(0, 0, 0, 1))
    # bleu_2_gram = corpus_bleu(hypotheses, list_of_refs, weights=(0, 1, 0, 0))
    # bleu_3_gram = corpus_bleu(hypotheses, list_of_refs, weights=(0, 0, 1, 0))
    # bleu_4_gram = corpus_bleu(hypotheses, list_of_refs, weights=(0, 0, 0, 1))
